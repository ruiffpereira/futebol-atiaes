...
export default function Rules({ onClose }: { onClose: () => void }) {
  return (
    <div className="m-fade" onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 80, background: 'rgba(8,30,18,.55)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: 'calc(env(safe-area-inset-top) + 18px) 14px calc(env(safe-area-inset-bottom) + 18px)', overflowY: 'auto' }}>
      <div className="m-pop" onClick={(e) => e.stopPropagation()} style={{ background: 'var(--surface)', borderRadius: 18, width: '100%', maxWidth: 680, overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: 'calc(100vh - 36px - env(safe-area-inset-top) - env(safe-area-inset-bottom))' }}>
        <div style={{ background: 'linear-gradient(135deg,var(--brand-dark),var(--brand))', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,.85)', fontWeight: 600, marginTop: 2 }}>Torneio Futebol 5 · Atiães em Movimento · 4ª Edição</div>
          </div>
        ...
        <div style={{ padding: '20px 22px 26px', overflowY: 'auto', flex: 1, minHeight: 0, color: 'var(--text)', fontSize: 14, lineHeight: 1.6 }}>
          ...
          <div style={{ background: 'var(--surface-2)', border: '1px solid var(--line-2)', borderRadius: 11, padding: '12px 14px', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
            <span style={{ background: 'var(--brand-tint)', color: 'var(--brand)', fontWeight: 700, padding: '5px 11px', borderRadius: 8, fontSize: 13 }}>Vitória = 3 pts</span>
            <span style={{ background: 'var(--surface-2)', color: 'var(--muted-2)', fontWeight: 700, padding: '5px 11px', borderRadius: 8, fontSize: 13 }}>Empate = 1 pt</span>
            <span style={{ background: 'var(--danger-tint)', color: 'var(--danger)', fontWeight: 700, padding: '5px 11px', borderRadius: 8, fontSize: 13 }}>Derrota/ausência = 0 pts</span>
          </div>
          <p style={{ margin: '0 0 18px', color: 'var(--muted-2)' }}>Nas fases a eliminar, empate é decidido por <b>penáltis em séries de 3</b>.</p>
          <Title>Prémios</Title>
