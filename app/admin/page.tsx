        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {t.players.map((p) => (
            <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#f6faf4', padding: '6px 9px', borderRadius: 8 }}>
              {p.number != null && <span style={{ minWidth: 20, textAlign: 'center', fontSize: 11.5, fontWeight: 800, color: '#fff', background: DGREEN, padding: '1px 5px', borderRadius: 5 }}>{p.number}</span>}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 10 }}>
        {draft.players.map((p) => (
          <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#f6faf4', padding: '5px 7px 5px 9px', borderRadius: 8 }}>
            ...
            <input type="number" inputMode="numeric" value={p.number ?? ''} placeholder="#" title="Número de camisola" onChange={(e) => setDraft((d) => ({ ...d, players: d.players.map((q) => (q.id === p.id ? { ...q, number: e.target.value === '' ? undefined : Math.max(0, Math.min(999, e.target.valueAsNumber || 0)) } : q)) }))} style={{ width: 44, flexShrink: 0, textAlign: 'center', border: '1px solid #d3e0d0', background: '#fff', fontSize: 13, fontWeight: 700, color: DGREEN, padding: '4px 3px', borderRadius: 6, outline: 'none' }} />
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {team?.players.map((p) => (
          <div key={p.id} style={{ display: 'flex', gap: 5 }}>
            <button onClick={() => apply(actions.addGoal(state, m.id, teamId!, p.id, silent))} style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: 7, justifyContent: 'space-between', border: '1.5px solid #e4ece0', background: '#fff', padding: '9px 11px', borderRadius: 10, fontSize: 14, fontWeight: 500 }}><span style={{ display: 'flex', alignItems: 'center', gap: 7, minWidth: 0 }}>{p.number != null && <span style={{ flexShrink: 0, minWidth: 20, textAlign: 'center', fontSize: 11.5, fontWeight: 800, color: '#fff', background: DGREEN, padding: '1px 5px', borderRadius: 5 }}>{p.number}</span>}<span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.name}</span></span><span style={{ flexShrink: 0, background: '#dcfce7', color: GREEN, fontWeight: 800, fontSize: 13, padding: '2px 6px', borderRadius: 6 }}>⚽{(m.scorers || []).filter((s) => s.player === p.id).length}</span></button>
